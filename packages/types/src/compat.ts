import { type ParamDef, type TypeExpr, asFuncType, isArrayType, isConsumerType, NamedType } from "./ast";
...
function visit(
  ...
  // A vector of consumers `Array[c1<T>]` may ground a `c1<T>` input (one channel per wire).
  ...
  const fromFunc = asFuncType(from);
  const toFunc = asFuncType(to);
  if (fromFunc && toFunc) {
    return visitFunc(catalog, params, fromFunc, toFunc, visited, depth, onMatch);
  }
  ...
  switch (from.kind) {
    ...
    case "self":
      return false;
    case "wildcard":
      if (from.bound === null || from.variance === null) {
        return true;
      }
      if (from.variance === "+") {
        if (to.kind === "wildcard") {
          if (to.variance === "+" && to.bound !== null) {
...
function visitFunc(
  ...
  const toFunc = asFuncType(to);
  if (!toFunc || from.params.length !== toFunc.params.length) {
    return false;
  }
  const paramsOk = from.params.every((formal, index) =>
    visit(catalog, params, formal, toFunc.params[index]!, visited, null, depth + 1, onMatch),
  );
  return paramsOk && visit(catalog, params, from.ret, toFunc.ret, visited, true, depth + 1, onMatch);
}
