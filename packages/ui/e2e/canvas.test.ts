...
test.describe("canvas", () => {
  ...
  test("renders a labeled block with a 32px icon and inset edge circles", async () => {
    ...
    await expect(host.locator('[data-testid="output-out"]')).toHaveAttribute("title", "(f32) -> void");
    await expect(host.locator(".block-port-name")).toHaveCount(0);
    await expect(host.locator('[data-testid="input-in"]')).toHaveAttribute("title", "(f32) -> void");
