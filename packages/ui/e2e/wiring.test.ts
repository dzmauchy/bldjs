...
test.describe("wiring", () => {
  ...
  test("shows port types only on the source output and compatible inputs while linking", async () => {
    ...
    await expect
      ...
      .toBe("(f32) -> void");
    expect(await portTypeText(page, "sin", "input-in")).toBe("(f32) -> void");
    expect(await portTypeText(page, "timer", "input-in")).toBe("(f32) -> void");
    expect(await portTypeText(page, "sin", "output-out")).toBeNull();
  ...
  test("keeps a second (f32) -> void wire on the same input", async () => {
    await placeBlock(page, "scope");
    await placeBlock(page, "scope");
    await placeBlock(page, "timer");
  ...
  test("adds extra ports for a second wire and removes them with the connector", async () => {
    ...
    await expect(scope.locator('[data-vector="out"] .block-port-name')).toHaveCount(0);
    await expect(scope.locator('[data-testid="output-out"]')).toHaveAttribute("title", "(f32) -> void");
    await expect(scope.locator('[data-testid="output-out[1]"]')).toHaveAttribute("title", "(f32) -> void");
    await clickPortHandle(page, "scope", "output-out");
    await expect.poll(async () => portTypeText(page, "scope", "output-out")).toBe("(f32) -> void");
  ...
  test("wires Scope → Sin and opens the chart", async () => {
    ...
    await expect(timerIn).toHaveAttribute("title", "(f32) -> void");
    expect(await timerIn.innerText()).not.toContain("(f32) -> void");
    await expect(nodeHost(page, "scope").locator('[data-testid="output-out"]')).toHaveAttribute("title", "(f32) -> void");
    const sinIn = nodeHost(page, "sin").locator('[data-testid="input-in"]');
    expect(await sinIn.innerText()).not.toContain("(f32) -> void");
    await expect(sinIn).toHaveAttribute("title", "(f32) -> void");
    await expect(nodeHost(page, "sin").locator('[data-testid="output-out"]')).toHaveAttribute("title", "(f32) -> void");
    const cosIn = nodeHost(page, "cos").locator('[data-testid="input-in"]');
    expect(await cosIn.innerText()).not.toContain("(f32) -> void");
    await expect(cosIn).toHaveAttribute("title", "(f32) -> void");
    await expect(nodeHost(page, "cos").locator('[data-testid="output-out"]')).toHaveAttribute("title", "(f32) -> void");
    expect(await portTypeText(page, "timer", "input-in")).toBeNull();
    ...
    await expect(page.locator('[data-testid="scope-caption"]')).toHaveText("5");
  ...
  test("opens a multi-axis chart after two vector channels run", async () => {
    ...
    await expect(page.locator('[data-testid="scope-modal"]')).toHaveAttribute("role", "region");
    await expect(page.locator('[data-testid="scope-footer"]')).toBeVisible();
    await expect(page.locator('[data-testid="scope-caption"]')).toHaveText("1");
    await expect(page.locator('[data-testid="scope-caption"]')).not.toContainText("timer(");
    await expect(page.locator('[data-testid="scope-close"]')).toBeVisible();
