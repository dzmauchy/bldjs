...
/** Scope capture window in seconds (`integer-range-parameter` `n`). */
export const DEFAULT_WINDOW_S = 30;
export const MIN_WINDOW_S = 10;
export const MAX_WINDOW_S = 600;
/** Scope meter interval in milliseconds (`integer-range-parameter` `m`). */
...
/**
 * Windowed scope capacity: `N` seconds at `1000 / M` samples per second.
 * Default `N = 30`, `M = 10` → 3000 measurements.
 */
export function sampleCap(n: number | string | undefined | null = DEFAULT_WINDOW_S, m: number | string | undefined | null = DEFAULT_METER_MS): number {
