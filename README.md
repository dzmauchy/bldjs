...
Diagrams load multiple XML type/block models (`packages/xml/src/resources/models/*.xml`, described by `packages/xml/src/resources/models/blocks.xsd` and `packages/xml/src/resources/models/blocks.md`). Wiring an output into an input grounds that input and infers the block's generic types. The builtin type library (`types.xml`) uses camelCase names: `double`, `float`, `int`, `int64`, `string`, `bool`, `unit`, function types `(T) -> R`, and `array[T]`. Blocks declare optional `<var>` type variables (with Prolog constraints such as `T:extends(h(F))`) and typed ports. Inference loads `packages/xml/src/blocks/prolog/types.pl` then `packages/xml/src/blocks/prolog/blocks.pl` into Trealla (`library(atts)`, `attr_unify_hook/2`). Rules are keyed by block `id`. The WASM runtime maps those onto WASM valtypes (`bool` → `i32`, `string` → js-string / `externref`). MoonBit emit still uses PascalCase (`Double`, `Unit`).
