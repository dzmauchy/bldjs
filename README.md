...
- Control Systems (`com.dauch.cs`): wire Timer (`com.dauch.cs.gen`) → Quantizer (`com.dauch.cs`) → Sin or Cos (`com.dauch.cs.transform`) → Oscilloscope (`com.dauch.cs.sink`). Every port is `c<f64>` (`DoubleConsumer`). Oscilloscope `in` is vararg, so several channels can share one plot: `oscilloscope(sin(quantizer(timer())), cos(timer()))`. Several `c<f64>` outputs may share one input; Run inserts a hidden `fork` that forwards each sample to every downstream. The WASM backend still lowers each sample to first-order `f64`.
- **Run** runs each block's [binaryen.js](https://github.com/AssemblyScript/binaryen.js) script from `src/resources/binaryen/blocks` into one module, emits wasm-gc (typed `call_ref`), starts one worker per generator, and parks with `memory.atomic.wait32` on a shared sample buffer. After Run, click Chart on Oscilloscope; the chart is a [Chart.js multi-axis line](https://www.chartjs.org/docs/latest/samples/line/multi-axis.html) with one dataset / y-axis per vararg channel.
