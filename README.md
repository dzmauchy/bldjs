...
- Phones load the page at 70% viewport scale (`initial-scale=0.7`) so toolbar, palette, and blocks take less of the screen. Desktop browsers ignore the viewport meta tag.
...
- **Run** asks SolutionBuilder to assemble the wired SolutionView: one [binaryen.js](https://github.com/AssemblyScript/binaryen.js) function per XML block from `src/lib/resources/binaryen/blocks`, connectors to wire them (GC array slots and `fork` on fan-in), then Binaryen emits wasm. Consumers are typed funcrefs (`(ref $c1_f64)`). It starts one worker thread per generator (Timer). That worker drives `tick` with `setInterval` (quantizer delay, default 10 ms) and writes samples into a shared buffer. The runner intercepts `c<?>` connector frequency after each tick; live wires set `animation-duration` from that Hertz (`1000 / hz`, clamped to 200–2500 ms) via inline style. After Run, click Chart on Scope; the chart is a [Chart.js multi-axis line](https://www.chartjs.org/docs/latest/samples/line/multi-axis.html) with one dataset / y-axis per vector channel.
...
- [Chart.js](https://www.chartjs.org/) for the scope (dark mode)
