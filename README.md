...
- Control Systems (`cs`): wire Timer → Quantizer → Sin → Oscilloscope. Port hints show the full nested consumer types (`c<c<c<f64>>>` on Timer). The WASM backend lowers those to first-order `f64` (`s<f64>`, `f1<f64, f64>`, `c1<f64>`), composed as `oscilloscope(sin(quantizer(timer())))`.
