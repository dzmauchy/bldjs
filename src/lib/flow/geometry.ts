...
/** Endpoints of an absolute SVG path (`M`/`L` plus `C` destinations). */
export function pathPolyline(d: string): Point[] {
  const points: Point[] = [];
  ...
  for (const token of tokens) {
    if (token === "M" || token === "L" || token === "C" || token === "m" || token === "l" || token === "c") {
      flush();
